...
func newUserCommand(app *App) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			info, err := app.API.GetUserInfo(ctx, uid, nil)
			if err != nil {
				return app.apiFailure(err, "获取用户信息失败", mode)
			}
			relation, err := app.API.GetUserRelationInfo(ctx, uid, nil)
			if err != nil {
				return app.apiFailure(err, "获取用户信息失败", mode)
			}
...
func newUserVideosCommand(app *App) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			videos, err := app.API.GetUserVideos(contextOrBackground(cmd.Context()), uid, count, nil)
			if err != nil {
